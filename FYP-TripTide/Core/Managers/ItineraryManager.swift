...
class ItineraryManager {
    ...
    func fetchAllItineraries(tripId: String, forceRefresh: Bool = false) async throws
        -> [DailyItinerary]
    {
        print(
            "🔍 ItineraryManager: Fetching all itineraries for trip \(tripId), forceRefresh: \(forceRefresh)"
        )
    ...
    func fetchItinerary(tripId: String, day: Int, forceRefresh: Bool = false) async throws
        -> DailyItinerary
    {
        print("🔍 ItineraryManager: Fetching itinerary for trip \(tripId), day \(day)")

        // Try to get all itineraries first (using cache if valid)
        let allItineraries = try await fetchAllItineraries(
            tripId: tripId, forceRefresh: forceRefresh)
    ...
    func createItinerary(tripId: String, day: Int, scheduledPlaces: [ScheduledPlaceDto])
        async throws -> DailyItinerary
    {
        print("🔧 ItineraryManager: Creating itinerary for trip \(tripId), day \(day)")

        let itinerary = try await itineraryService.createItinerary(
            tripId: tripId, day: day, scheduledPlaces: scheduledPlaces)
    ...
    func updateItinerary(tripId: String, day: Int, scheduledPlaces: [ScheduledPlaceDto])
        async throws -> DailyItinerary
    {
        print("🔧 ItineraryManager: Updating itinerary for trip \(tripId), day \(day)")

        let itinerary = try await itineraryService.updateItinerary(
            tripId: tripId, day: day, scheduledPlaces: scheduledPlaces)
    ...
    private func updateCacheWithItinerary(tripId: String, itinerary: DailyItinerary) {
        ...
        if var cachedItineraries = itinerariesCache[tripId] {
            ...
            if let index = cachedItineraries.firstIndex(where: {
                $0.dayNumber == itinerary.dayNumber
            }) {
            ...
            print(
                "📦 ItineraryManager: Updated cache for trip \(tripId), day \(itinerary.dayNumber)")
        } else {
    ...
    /// Deletes an itinerary for a specific day
    func deleteItinerary(tripId: String, day: Int) async throws {
        print("🗑️ ItineraryManager: Deleting itinerary for trip \(tripId), day \(day)")

        try await itineraryService.deleteItinerary(tripId: tripId, day: day)

        // Update local cache
        if var cachedItineraries = itinerariesCache[tripId] {
            cachedItineraries.removeAll(where: { $0.dayNumber == day })
            itinerariesCache[tripId] = cachedItineraries
            lastFetchTime[tripId] = Date()
            print("📦 ItineraryManager: Updated cache after deletion for trip \(tripId), day \(day)")
        }
    }

    /// Checks if an itinerary should be deleted (when empty) and deletes it if needed
    func handleEmptyItinerary(tripId: String, day: Int, scheduledPlaces: [ScheduledPlaceDto])
        async throws
    {
        print(
            "🔍 ItineraryManager: Checking if itinerary should be deleted for trip \(tripId), day \(day)"
        )

        if scheduledPlaces.isEmpty {
            // If there are no places, delete the itinerary
            try await deleteItinerary(tripId: tripId, day: day)
            print("✅ ItineraryManager: Deleted empty itinerary for trip \(tripId), day \(day)")
        } else {
            // Otherwise update it with the provided places
            _ = try await updateItinerary(
                tripId: tripId, day: day, scheduledPlaces: scheduledPlaces)
        }
    }
}
